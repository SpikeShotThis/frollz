...
export class Migration20260429150000 extends Migration {
  override async up(): Promise<void> {
    this.addSql(
      'create table "film_supplier" ("id" serial primary key, "user_id" int not null, "name" text not null, "normalized_name" text not null, "contact" text null, "email" text null, "website" text null, "notes" text null, "active" boolean not null default true, "rating" int null, constraint "film_supplier_user_id_foreign" foreign key ("user_id") references "user" ("id") on update cascade);'
    );
    ...
    this.addSql('alter table "film_lot" add column "rating" int null;');
    this.addSql('update "film_lot" set "obtained_date" = "created_at" where "obtained_date" is null;');
    this.addSql('alter table "film_lot" add constraint "film_lot_supplier_id_foreign" foreign key ("supplier_id") references "film_supplier" ("id") on update cascade on delete set null;');
  ...
  override async down(): Promise<void> {
    ...
    this.addSql('drop table if exists "film_supplier" cascade;');
  }
}
